...
class Mp_Tutor_Lms_Admin {
	...
	/**
	 * Register currencies Rupiah.
	 *
	 * @since    1.0.0
	 */
	public function register_edd_currencies( $currencies ) {
		$currencies['Rp'] = __('Indonesia (IDR)', 'mp-tutor-lms');
		return $currencies;
	}
	...
	public function set_edd_decimals() {
		if ( edd_get_currency() == 'Rp' ) {
			return 0;
		}
	}
	 ...
	 * @since    1.0.0
	 */
	public function enqueue_style_preview() {

		// Style Admin
		wp_enqueue_style( 'mp-preview-admin', plugin_dir_url( __FILE__ ) . 'css/style-preview.css', array(), $this->version, 'all' );
		
	}

	/**
	 * Register the JavaScript for the preview area.
	 *
	 * @since    1.0.0
	 */
	public function enqueue_scripts_preview() {
		...
		// Scripts Preview
		wp_enqueue_script( 'mp-js-preview', plugin_dir_url( __FILE__ ) . 'js/script-preview.js', array( 'customize-preview' ), $this->version, true );

	}

	/**
	 * Register the JavaScript for the controls area.
	 *
	 * @since    1.0.0
	 */
	public function enqueue_scripts_controls() {
		...
		// Scripts Controls
		wp_enqueue_script( 'mp-js-controls', plugin_dir_url( __FILE__ ) . 'js/script-controls.js', array(), $this->version, true );

	}

	/**
	 * Register Widget Payment
	 *
	 * @since    1.0.0
	 */
	function Mp_Tutor_Lms_Widgets_Register() {
		// Footer 1
		register_sidebar(
			array(
				'name'          => __( 'Footer #1 (MP Tutor LMS)', 'mp-tutor-lms' ),
				'id'            => 'mp_tutor_lms_footer1',
				'description'   => __( 'Footeer #1', 'mp-tutor-lms' ),
		...
		register_sidebar(
			array(
				'name'          => __( 'Footer #2 (MP Tutor LMS)', 'mp-tutor-lms' ),
				'id'            => 'mp_tutor_lms_footer2',
				'description'   => __( 'Footeer #2', 'mp-tutor-lms' ),
				'before_widget' => '<section id="%1$s" class="widget %2$s">',
				'after_widget'  => '</section>',
				'before_title'  => '<h4 class="widget-title-footer">',
				'after_title'   => '</h4>',
		...
		register_sidebar(
			array(
				'name'          => __( 'Footer #3 (MP Tutor LMS)', 'mp-tutor-lms' ),
				'id'            => 'mp_tutor_lms_footer3',
				'description'   => __( 'Footeer #3', 'mp-tutor-lms' ),
				'before_widget' => '<section id="%1$s" class="widget %2$s">',
				'after_widget'  => '</section>',
				'before_title'  => '<h4 class="widget-title-footer">',
				'after_title'   => '</h4>',
		...
		register_widget( 'Mp_Tutor_Lms_Follow_Us' );

		register_widget( 'Mp_Tutor_Lms_Payment' );
	}
	...
	public function register_menu() {
		register_nav_menus( 
	    	array(
	            'mp-tutor-lms-header' => __( 'MP Tutor LMS Header', 'mp-tutor-lms' ),
	    	)
	    );
	}
		...
		<div class="wrap mptutorlms-license-window">
			<form method="post" action="options.php" class="elementor-license-box">
				...
				<h3>
					...
					<input id="mptutorlms_license_key" name="mptutorlms_license_key" type="text" value="" placeholder="<?php esc_attr_e( 'masukkan kode lisensi di sini'); ?>" />
					</div>
					</div>
            ...
            <div class="mptutorlms-license-alert-success">
				...
				<?php if ( in_array( $status, array( 'expired' ) ) ) : ?>
					<p class="elementor-admin-alert elementor-alert-danger"><?php printf( __( '<strong>Lisensi telah Expired.</strong> <a href="%s" target="_blank">Perpanjang Lisensi Sekarang</a> supaya tetap mendapatkan update, premium support dan akses MP Tutor LMS template library.' ), 'https://user.brandmarketers.id' ); ?></p>
				<?php endif; ?>
	...
	/**
	 * Check lisence is active or not
	 * 
	 * @return boolean [description]
	 */
    public function is_active() {
		$license_status = get_option( 'mptutorlms_license_key_status' );
